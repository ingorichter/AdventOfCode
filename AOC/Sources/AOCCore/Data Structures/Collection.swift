...
public extension Collection {
    ...
    func combinations(choose k: Int? = nil) -> AnySequence<[Element]> {
        ...
        if let k = k {
            return AnySequence(combinations(ofCount: k))
        } else {
            var s = AnySequence(combinations(ofCount: 1))
            if count >= 2 {
                for count in 2...count {
                    let combo = combinations(ofCount: count)
    ...
    func split(on isBoundary: (Element) -> Bool, includeEmpty: Bool = true) -> Array<SubSequence> {
        split(omittingEmptySubsequences: !includeEmpty, whereSeparator: isBoundary)
    }
...
public extension BidirectionalCollection {
    ...
    func trimming(_ matches: (Element) -> Bool) -> SubSequence {
        removingFirst(while: matches).removingLast(while: matches)
    }
}
