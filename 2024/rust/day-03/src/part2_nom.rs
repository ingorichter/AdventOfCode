...
//         // Find the next "don't()" and "do()" positions relative to `search_start`
//         let dont_match = dont_re.find(&input[search_start..]);
//         let do_match = do_re.find(&input[search_start..]);

//         println!("search_start: {}", search_start);
//         dbg!(search_start, dont_match, do_match, collecting);

//         match (dont_match, do_match) {
//             (Some(dont), Some(do_)) => {
//                 if dont.start() < do_.start() {
//                     // process everything before don't()
//                     if collecting {
//                         let before_dont = &input[search_start..search_start + dont.start()];
//                         dbg!(&before_dont);
//                         for cap in mul_re.captures_iter(before_dont) {
...
//                         dbg!(&results);
//                     }
//                     search_start += do_.end();
//                     collecting = true;
//                 } else {
//                     // Process everything and do() and before don't()
//                     // if collecting {
//                         let after_do = &input[search_start..search_start + dont.start()];
//                         dbg!(&after_do);
//                         for cap in mul_re.captures_iter(after_do) {
//                             if let (Ok(x), Ok(y)) = (cap[1].parse::<i32>(), cap[2].parse::<i32>()) {
//                                 results.push((x, y));
//                             }
//                         }
...
//                     // dbg!(&results);
//                     collecting = false;
//                     search_start += dont.end(); // Move past "don't()"
//                 }
//             }

//             // (Some(dont), Some(do_)) if do_.start() < dont.start() => {
//             //     // Process "do()" before the next "don't()"
//             //     if collecting {
//             //         let after_do = &input[search_start + do_.end()..search_start + dont.start()];
//             //         for cap in mul_re.captures_iter(after_do) {
//             //             if let (Ok(x), Ok(y)) = (cap[1].parse::<i32>(), cap[2].parse::<i32>()) {
//             //                 results.push((x, y));
//             //             }
//             //         }
//             //     }
//             //     search_start += do_.end(); // Move past "do()"
//             //     collecting = false;
//             // }
//             (Some(dont), Some(_)) => {
//                 // Handle "don't()" - Stop collecting until after the next "do()"
//                 collecting = false;
//                 search_start += dont.end();
//             }
//             (None, Some(do_)) => {
//                 // Process "do()" when no more "don't()" exists
//                 if collecting {
//                     let after_do = &input[search_start + do_.end()..];
//                     for cap in mul_re.captures_iter(after_do) {
//                         if let (Ok(x), Ok(y)) = (cap[1].parse::<i32>(), cap[2].parse::<i32>()) {
//                             results.push((x, y));
//                         }
...
//             }
//             (Some(dont), None) => {
//                 // Handle "don't()" with no following "do()"
//                 if collecting {
//                     let before_dont = &input[search_start..search_start + dont.start()];
//                     for cap in mul_re.captures_iter(before_dont) {
//                         if let (Ok(x), Ok(y)) = (cap[1].parse::<i32>(), cap[2].parse::<i32>()) {
...
//             }
//             (None, None) => {
//                 // No more "don't()" or "do()" - collect remaining `mul(x,y)`
//                 if collecting {
//                     let remaining = &input[search_start..];
//                     for cap in mul_re.captures_iter(remaining) {
//                         if let (Ok(x), Ok(y)) = (cap[1].parse::<i32>(), cap[2].parse::<i32>()) {
//                             results.push((x, y));
//                         }
//                     }
